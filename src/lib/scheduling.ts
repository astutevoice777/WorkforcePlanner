...
export async function generateScheduleWithAI(request: AIScheduleRequest): Promise<AIScheduleResponse> {
  ...
  days.forEach((day, dayIndex) => {
    ...
    roles.forEach(role => {
      const availableStaffForRole = availableStaffIds.filter((staffId: string) => {
      ...
      for (let i = 0; i < staffToSchedule; i++) {
        ...
        const availability = staffAvailability[staffId][day as keyof typeof staffAvailability[staffId]];
