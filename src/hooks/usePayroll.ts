...
export function usePayroll() {
  ...
  // Fetch payroll periods with entries (mock implementation)
    ...
    try {
      ...
      const { data: mockPeriods, error } = await supabase
        .from('payroll_periods')
        .select('id, business_id, period_start, period_end, status, total_hours, total_gross_pay, total_deductions, total_net_pay, created_at, updated_at, entries')
